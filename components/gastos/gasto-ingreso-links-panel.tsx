...
interface Props {
  gastoId: string
  gastoCurrency: Currency
}

export function GastoIngresoLinksPanel({ gastoId, gastoCurrency }: Props) {
  ...
  const [linkCurrency, setLinkCurrency] = useState<Currency>(gastoCurrency)
  const [saving, setSaving] = useState(false)

  const fetchLinks = useCallback(async () => {
    setLoading(true)
    try {
      const data = await loadLinksForGasto(gastoId)
      setLinks(data)
    } finally {
      setLoading(false)
    }
  ...
  const openAdd = async () => {
    const opts = await loadIngresoOptions()
    setIngresoOptions(opts)
    setSelectedIngresoId("")
    setAttributedAmount("")
    setLinkCurrency(gastoCurrency)
    setShowAdd(true)
  }

  const handleAdd = async () => {
    if (!selectedIngresoId || !attributedAmount) return
    setSaving(true)
    try {
  ...
  return (
    <div className="border-t border-gray-200 bg-gray-50 px-3 py-3">
      <div className="mb-2 flex items-center justify-between">
        <span className="text-xs font-bold uppercase text-gray-500">Ingresos que financian este gasto</span>
        <button
          onClick={openAdd}
      ...
      {loading ? (
        ...
        <div className="py-2 text-xs text-gray-400">Sin ingresos vinculados.</div>
      ) : (
        <div className="flex flex-col gap-1">
          {links.map((link) => (
            <div key={link.id} className="flex items-center justify-between rounded border border-gray-200 bg-white px-3 py-1.5 text-xs">
              <span className="font-medium">{link.ingresoName ?? link.ingresoId}</span>
        ...
        <Dialog open onOpenChange={(o) => !o && setShowAdd(false)}>
          <DialogContent className="max-w-sm rounded-none border-2 border-black p-0">
            <DialogHeader className="border-b-2 border-black bg-black px-4 py-3">
              <DialogTitle className="font-bold italic text-white">Vincular ingreso</DialogTitle>
            </DialogHeader>
            <div className="flex flex-col gap-4 px-4 py-4">
              <div className="flex flex-col gap-1">
                <Label className="text-xs font-bold uppercase">Ingreso *</Label>
                <Select value={selectedIngresoId} onValueChange={setSelectedIngresoId}>
                  <SelectTrigger className="border-2 border-black">
                    <SelectValue placeholder="Seleccionar ingreso..." />
                  </SelectTrigger>
                  <SelectContent>
                    {ingresoOptions.length === 0 ? (
                      <SelectItem value="_none" disabled>Sin ingresos disponibles</SelectItem>
                    ) : (
                      ingresoOptions.map((i) => (
                        <SelectItem key={i.id} value={i.id}>
                          {i.name}
                          <span className="ml-2 text-xs text-gray-400">
                            ({formatAmount(i.amount, i.currency as Currency)} {i.currency})
                          </span>
                        </SelectItem>
                      ))
                    )}
            ...
            <DialogFooter className="border-t-2 border-black px-4 py-3">
              ...
              <Button
                ...
                onClick={handleAdd}
                disabled={saving || !selectedIngresoId || !attributedAmount}
              >
